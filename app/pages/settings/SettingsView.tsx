      ...
      <section className="nothing-hairline-b relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-12">
          ...
          <p className="text-sm text-muted-foreground mt-5 max-w-xl">
            Update your identity, contact email, and password. UI only — no calls are wired.
          </p>
        </div>
      </section>
...
function FormShell({ title, hint, children, onSubmit }: {
  title: string;
  hint: string;
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" className="nothing-btn nothing-btn--signal">Save changes ↗</button>
        <button type="reset" className="nothing-btn">Reset</button>
      </div>
    </motion.form>
  );
}
...
function Field({ label, hint, ...props }: { label: string; hint?: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      ...
      {hint && (
        <p className="nothing-mono text-[10px] uppercase tracking-[0.16em] text-ink-mute mt-2">{hint}</p>
      )}
    </div>
  );
}
...
function ProfileForm() {
  const [name, setName] = useState("Guest Operator");
  const [handle, setHandle] = useState("guest");
  const [bio, setBio] = useState("Designing in monochrome.");
  return (
    <FormShell title="Profile" hint="public identity" onSubmit={(e) => e.preventDefault()}>
      <div className="grid sm:grid-cols-2 gap-5">
        <Field label="Display name" value={name} onChange={(e) => setName(e.target.value)} />
        <Field label="Handle" value={handle} onChange={(e) => setHandle(e.target.value)} hint="halftone.dev/@handle" />
      </div>
      <div>
        <label className="nothing-label">Bio</label>
        <textarea className="nothing-textarea" value={bio} onChange={(e) => setBio(e.target.value)} placeholder="A short line about you" />
      </div>
    </FormShell>
  );
}
...
function AccountForm() {
  const [email, setEmail] = useState("guest@halftone.dev");
  const [recovery, setRecovery] = useState("");
  return (
    <FormShell title="Account" hint="contact + recovery" onSubmit={(e) => e.preventDefault()}>
      <Field label="Email (Gmail)" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@gmail.com" />
      <Field label="Recovery email" type="email" value={recovery} onChange={(e) => setRecovery(e.target.value)} placeholder="backup@gmail.com" hint="used if you lose access" />
    </FormShell>
  );
}
...
function SecurityForm() {
  ...
  return (
    <FormShell title="Security" hint="password" onSubmit={(e) => e.preventDefault()}>
